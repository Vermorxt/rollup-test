...
export function itSupportsInputWrapperProps<P>(Component: React.ComponentType<P>, requiredProps: P, name: string) {
  ...
  it('supports description prop', async () => {
    const { getByText } = await renderWithAct(<Component {...requiredProps} description="Test description" />)
  ...
  it('supports labelProps', async () => {
    await renderWithAct(<Component {...requiredProps} label="Test label" labelProps={{ 'data-test': 'test-data' }} />)
