...
export function itSupportsSx<P>(Component: React.ComponentType<P>, requiredProps: P) {
  it('supports sx', async () => {
    ...
    const fn = () => styles
    const { container: withFunction } = await renderWithAct(<Component {...requiredProps} sx={fn} />)
    const { container: withObject } = await renderWithAct(<Component {...requiredProps} sx={styles} />)
